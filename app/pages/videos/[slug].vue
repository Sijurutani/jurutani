...
// Helper to get YouTube thumbnail for related videos
...
const shareUrl = computed(() => {
  if (typeof window !== 'undefined') {
    return `${window.location.origin}/videos/${slug.value}`
  }
  return `https://jurutani.com/videos/${slug.value}`
})
      ...
      <div v-else-if="hasData">
        ...
        <section v-if="relatedVideos.length > 3" class="mt-12">
          ...
          <div class="mt-8 flex justify-center">
            <NuxtLink
              to="/videos"
              class="inline-flex items-center space-x-2 px-5 py-2.5 text-sm font-semibold bg-linear-to-r from-green-600 to-pink-700 hover:from-green-700 hover:to-pink-800 text-white rounded-lg transition-all duration-200 hover:shadow-lg hover:shadow-green-500/25 transform hover:-translate-y-0.5"
            >
