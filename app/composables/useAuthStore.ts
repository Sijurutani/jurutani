...
export const useAuthStore = () => {
  ...
  const roleLabel = computed(() => {
    return (
      Enum.UserRole.find((r) => r.value === profile.value?.role)?.label ||
      'Pengguna'
    )
  })
    ...
    try {
      ...
      if (authError) {
        ...
        if (
          ...
          msg =
            'Pendaftaran akun baru sedang dinonaktifkan. Silakan coba lagi nanti.'
      ...
      if (!data.user.email_confirmed_at) {
        return {
          ...
          message:
            'Akun berhasil dibuat! Silakan cek email Anda untuk mengkonfirmasi akun.',
        }
      }
